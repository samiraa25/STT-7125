...
g1 <-ggplot(earinf, aes(x = Swim, fill = factor(Infec))) +
  geom_bar(position = "fill") +
  labs(title = "Proportion d'infection selon Swim",
       y = "Proportion", fill = "Infection") +
  theme_minimal()

g2 <-ggplot(earinf, aes(x = Loc, fill = factor(Infec))) +
  geom_bar(position = "fill") +
  labs(title = "Proportion d'infection selon Loc",
       y = "Proportion", fill = "Infection") +
  theme_minimal()

g3 <-ggplot(earinf, aes(x = Sex, fill = factor(Infec))) +
  geom_bar(position = "fill") +
...
summary(earinf);
#par(mfrow=c(1,2))
g1;g2;g3; boxplot(Age ~ Infec, data = earinf, main="Âge selon infection", ylab="Âge")
```
...
Les méthodes de sélection de variables, forward, backward et stepwise sélectionnent les variables Swim et Loc comme étant les plus significatives dans le modèle. Voici la méthode forward à titre d'exemple ainsi que les AIC finaux des 3 méthodes. On a le modèle suivant: $Y_i = \beta_0 + \beta_1 Swim_{i} + \beta_2 Loc_{i}$
```{r}
forward_1 <- step(modele_min_1, scope = list(lower = modele_min_1, upper = modele_max_1), 
              direction = "forward", trace = TRUE)
...
On obtient le modèle suivant: $Y_i = \beta_0 + \beta_1 Swim_{i} + \beta_2 Loc_{i} + \beta_3 Sex_{i}+ \beta_4 Loc*Sex_{i}$.
...
Le modèle est $Y_i = -0.41053576 + 0.41509704Swim_{i} + 0.03306821 Loc_{i} + -0.46438016 Sex_{i}+ 1.05834579 Loc*Sex_{i}$.<br><br>
On peut donc interpréter celui-ci avec $e^{\beta_j}$:
Les nageurs occasionnels ont $e^{0.41509704} = 1.51$, donc 51% plus de chances d'infection que les nageurs fréquents,
nager dans dans quelque part d'autre que dans la mer aumente le risque de 103%, les hommes ont 159% moins de chance d'avoir une infection et deplus, ils ont 2.88 fois plus de chances de'infection s'ils nagent hors plage d'attraper une infection.
...
rapport <- deviance_residuelle / df
...
Effectuons des comparaisons pour des modèles emboités et non emboités afin de valider que le modèle sélectionné est un bon modèle
```{r, include=FALSE}
modele_complet<-glm(Infec ~ Swim + Loc + Sex + Loc:Sex, data=earinf, family=binomial(link="logit"))
...
En déterminant l'aire sous la courbe ROC, on obtient une aire de 63.3% avec une erreur-type de 0.032. <br>
Le modèle ne prédit pas bien l'infection de l'oreille à travers ce jeu de données
```{r}
